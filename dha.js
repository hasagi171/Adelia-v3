...
module.exports = dha = async (dha, mek) => {
	try {
		...
		const s = groupAdmins.includes(sender) || false
      ...
      // Anti link
        if (isGroup && isAntiLink && isBotGroupAdmins){
            if (budy.match(/(https:\/\/chat.whatsapp.com)/gi)) {
                reply(`*「 GROUP LINK DETECTOR 」*\nSepertinya kamu mengirimkan link grup, maaf kamu akan di kick dalam hitungan ke 3`)
                               await sleep(2000)
                dha.sendMessage(from, `1`, text)
                await sleep(2000)
                dha.sendMessage(from, `2`, text)
                await sleep(2000)
                dha.sendMessage(from, `3`, text)
                await sleep(3000)
...
case 'linkgc':
				...
				break
            case 'kick':
             if (s && botNumber) return reply(mess.only.admin)
             if (!isGroup) return reply(mess.only.group)
...
case 'caripesan':  //by ANU TEAM
       ...
       case 'bugreport':
              ...
              dha.sendMessage('6285339189356@s.whatsapp.net',`*Bug Report:* ${teks}`, text)
              break
       case 'readall':
       ...
       case 'welcome':
        ...
        case 'groupsetting':
 		...
 		case 'group':
					...
					if (s && botNumber) return reply(ind.admin())
					if (!isBotGroupAdmins) return  reply(ind.badmin())
					if (args[0] === 'buka') {
...
//------------------< Menunya Bang:v >-------------------
      case 'dana':
         ...
         case 'pulsa':
             ...
             break              
      case 'infoig':
             reply(`Follow Instagram Owner Ya : https://www.instagram.com/_marselhasagi`)
             break
      case 'grupbot':
